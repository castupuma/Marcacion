...
function listadosGrupoRiesgo() {
  ...
  $.ajax({
    data: dat,
    url: g_url + "apicalendar/api/gruporiesgo.php",
    type: "POST",
    dataType: "json",
    contentType: "application/json",
    processData: false,
        ...
        _datares.forEach((elemento, indice) => {
          if (elemento.estado == "ACTIVO") {
            $("#example1 tbody").append(
              "<tr>" +
                '<td><span class="badge bg-primary" style="width: 100%;" onclick="listadoGrupoRiesgoId(' +
                elemento.id +
                ')">' +
                elemento.id +
                "</span></td>" +
...
function listadoGrupoRiesgoId(idgruporiesgo) {
  var dat = JSON.stringify({
    proceso: 2,
    p1: idgruporiesgo,
    p2: 2,
...
function registrarGrupoRiesgo() {
  var dat = JSON.stringify({
    proceso: 3,
    p1: $("#idgruporiesgo").val(),
    p2: $("#descripcion").val(),
    p3: $("#estado").val(),
    p4: 0,
    p5: 0,
