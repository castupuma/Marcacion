      ...
      for (let i = 1; i <= ultimoDia.getDate(); i++) {
        ...
        if (i == 1) {
          ...
          for (let j = diainicio; j <= 6; j++) {
            fila +=
              '<td style="padding: 0rem"> <div class="btn btn-app col-12 button-calendar"> <span class="badge bg-danger" style="right: -1px; top: -1px">' +
              _datares[i].turno +
              "</span> " +
              Number(i).toFixed(0);
            if (_datares[i].clase != "") {
...
function cargarcalendario(_idusuario, _title) {
  ...
  $.ajax({
        ...
        if (i == 1) {
          ...
          for (let j = diainicio; j <= 6; j++) {
            if (_datares[i].turno ) {
              ...
              Number(i).toFixed(0) + ','+ _datares[i].rol +
            ...
            fila +=
              '<td style="padding: 0rem"> <div class="btn btn-app col-12 button-calendar" data-toggle="modal" )"> '+Number(i).toFixed(0);;
    ...
    error: function () {
      alert("Ocurrio un error en el servidor..2");
    },
  });
...
function listadoPersonal() {
  ...
  $.ajax({
    ...
    error: function () {
      alert("Ocurrio un error en el servidor ..5");
    },
  });
}
...
function listadoServicio() {
  ...
  ({
    data: dat,
    url: g_url + "apicalendar/api/horario_2.php",
    type: "POST",
    dataType: "json",
    contentType: "application/json",
    processData: false,
    ...
    error: function () {
      alert("Ocurrio un error en el servidor ..20");
    },
  });
}
...
function listadoActividades() {
  ...
  $.ajax({
    ...
    error: function () {
      alert("Ocurrio un error en el servidor ..25");
    },
  });
}
function listadoPersonalServicio() {
  var dat = JSON.stringify({
    proceso: 10,
    periodo: g_periodo,
    mes: $("#idmes").val(),
  ...
  $.ajax({
    ...
    error: function () {
      alert("Ocurrio un error en el servidor ..10");
    },
  });
}

function seleccionardia(T_dia,rol) {
  g_dia = T_dia;
  id_rol = rol;
...
function listadoHoras(_idturno) {
  ...
  $.ajax({
    ...
    error: function () {
      alert("Ocurrio un error en el servidor .. 11");
    },
  });
}
...
function listadoAreaServicio() {
  ...
  $.ajax({
    ...
    error: function () {
      alert("Ocurrio un error en el servidor ..8");
    },
  });
}
...
function RegistroHorario() {
  ...
  $.ajax({
    ...
    error: function () {
      alert("Ocurrio un error en el servidor Registro..");
    },
  });
}
          ...
          if (elemento.Autorizado == 0) {
            $("#tablaProgramadoDia tbody").append(
              "<tr>" +
                ...
                "</span>" +
                "</td>" +
                '<td style="padding-right: 0.2rem;" onclick="eliminarActividad(' +
                elemento.nro +
...
function eliminarActividad(nro) {
  var dat = JSON.stringify({
    proceso: 1,
    p1: nro,
    p2: 2,
...
var mensaje = confirm("¿Seguro que desea Eliminar la actividad?");
