...
class HorarioDAO
{
    ...
    public function listaProgramadosDiaServicio($periodo, $mes, $dia, $servicio)
    {
        try {
            ...
            WHERE r.periodo = :periodo AND r.mes = :mes AND r.dia = :dia AND r.idarea = :area and r.idestado=0;";
            $database->query($sql);
    ...
    public function listaProgramadosDia($periodo, $mes, $dia, $servicio)
    {
        try {
            ...
            WHERE r.periodo = :periodo AND r.mes = :mes AND r.dia = :dia and s.idservicio = :servicio and r.idestado=0 order by s.idarea asc;";
            $database->query($sql);
            $database->bind(':periodo', $periodo);
            $database->bind(':mes', $mes);
            $database->bind(':dia', $dia);
    ...
    public function listaProgramadosServicio($periodo, $mes, $idplanilla)
    {
        try {
            ...
            LEFT JOIN p_turnos t ON t.idturno=r.idturno
            WHERE r.periodo = :periodo AND r.mes = :mes AND r.idplanilla = :idplanilla and r.idestado=0
            order by 1 asc;";
            $database->query($sql);
            $database->bind(':periodo', $periodo);
            $database->bind(':mes', $mes);
    ...
    public function listaProgramado($periodo, $mes, $idusuario)
    {
        try {
            ...
            $sql = "SELECT r.idrol as id, d.iddia as dia
            ...
            FROM p_diassemana d 
            left join p_rolprogramacion r  on d.iddia=r.dia and r.periodo = :periodo AND r.mes = :mes AND r.idplanilla = :idusuario and r.idestado=0
            LEFT JOIN p_area s ON s.idarea =r.idarea
            order by 1 asc;";
            $database->query($sql);
            $database->bind(':periodo', $periodo);
